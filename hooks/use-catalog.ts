 ...
 *
 * Pages resolve the catalogue on the server, so these exist for anything that
 * needs it after hydration — live stock on a product page, a client-side
 * filter, a re-check after the cart changes. Each hook is disabled when no
 * backend is configured, so it resolves to `undefined` and the caller keeps its
 * static data instead of spinning forever.
 */
