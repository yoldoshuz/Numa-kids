 ...
 * `t("products.<slug>.name")` / `articles.<slug>`, so this overlay is the only
 * way live copy reaches a component — no component knows the backend exists,
 * and an unreachable backend simply leaves the bundled copy in place.
 *
 ...
 * Without it `getTranslations` throws MISSING_MESSAGE and the page answers 500.
 */
function skeleton(template: Dict, headline: string, prose: string): Dict {
  ...
  if ("shortName" in out) out.shortName = headline;
  if ("title" in out) out.title = headline;
  return out;
}

/**
 * Builds `{ products: {...}, articles: {...} }` for one locale.
 *
 ...
 * copy for — otherwise a blanked body would wipe out a shipped article.
 *
...
export async function buildContentMessages(
  ...
  const bundledProducts = asDict(base.products);
  const bundledArticles = asDict(base.articles);
